...
$('document').ready(() => {
	...
	data.forEach(item => {
		...
		const card = $("<div class='card text-center work'>")
			.append($("<img class='card-img img-portfolio img-flud'>")
				.attr('src', item.src)
				.attr('alt', item.title)
			)
			.append($("<div class='card-img-overlay'>")
				.append(cardBody)
			);

		const element = $("<div class='col-12 col-sm-6 col-md-12 col-lg-6 col-xl-6'>")
			.append(card)
		$('.portfolio-grid').append(element);
